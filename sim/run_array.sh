#!/bin/bash
# =============================================================================
# run_array.sh  --  SLURM job array for v1_batch5_lock (5 seeds, 1 node each)
#
# Each array task gets its OWN node with 20 MPI ranks -- no core contention.
# Pre-requisite:  python gen_lock_cfgs.py   (generates the per-seed cfg JSONs)
#
# Submit from sim/:
#   sbatch run_array.sh
# =============================================================================

#SBATCH --job-name=v1_lock
#SBATCH --array=0-4
#SBATCH --nodes=1
#SBATCH --ntasks=20
#SBATCH --mem=50G
#SBATCH --time=01:00:00
#SBATCH --output=../data/v1_batch5_lock/slurm_%A_%a.out
#SBATCH --error=../data/v1_batch5_lock/slurm_%A_%a.err
...
if [ ! -f "${CFG_JSON}" ]; then
    ...
    echo "Run  python gen_lock_cfgs.py  first."
    exit 1
fi
