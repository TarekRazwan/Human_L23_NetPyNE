...
#= Analysis Parameters
#===============================
transient = 0 #used for plotting and analysis
...
def plot_network_somas(OUTPUTPATH):
	...
	for pop in popnames:
		for i in range(0,len(popDataArray[pop]['gid'])):
			ax.scatter(popDataArray[pop]['x'][i],popDataArray[pop]['y'][i],popDataArray[pop]['z'][i], c=pop_colors[pop], s=8)
			ax.set_xlim([-300, 300])
			ax.set_ylim([-300, 300])
...
def plot_somavs(network,VOLTAGES):
	...
	for i, pop in enumerate(network.populations):
		for v in range(0,len(VOLTAGES['volts'][i])):
			n_samples = len(VOLTAGES['volts'][i][v])
			tvec = np.arange(0, network.tstop, network.tstop/n_samples)
			ax = plt.subplot2grid((2*len(VOLTAGES['volts']), len(VOLTAGES['volts'][i])-1), (ii, 0), rowspan=1, colspan=1, frameon=False)
...
if RANK ==0:
	...
	if plotsomavs:
		...
		np.save(os.path.join(OUTPUTPATH,'VOLTAGES_Seed'+str(GLOBALSEED)+'.npy'), VOLTAGES)
#===============================
# Kant
#===============================
pop_colors = {'HL23PYR':'k', 'HL23SST':'red', 'HL23PV':'green', 'HL23VIP':'yellow'}
