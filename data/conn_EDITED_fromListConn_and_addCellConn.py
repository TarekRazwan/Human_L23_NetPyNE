...
"""
    ...
    if 'loc' in connParam and isinstance(connParam['loc'], list):
        connParam['locFromList'] = list(connParam['loc'])  # if loc is a list, copy to locFromList
    if 'sec' in connParam and isinstance(connParam['sec'], list):
        connParam['secFromList'] = list(connParam['sec'])  # if sec is a list, copy to secFromList
    ...
    for iconn, (relativePreId, relativePostId) in enumerate(connParam['connList']):  # for each postsyn cell
        ...
        if postCellGid in self.gid2lid:  # check if postsyn is in this node's list of gids
            ...
            if 'secFromList' in connParam: connParam['sec'] = connParam['secFromList'][iconn]
...
def _addCellConn(self, connParam, preCellGid, postCellGid, preCellsTags={}):
    ...
    paramPerSynMech = ['weight', 'delay', 'loc', 'sec']
    for i, synMech in enumerate(connParam.get('synMech')):
        ...
        # print(connParam)
        # print(finalParam['secSynMech'][i])
        # print(finalParam['locSynMech'])

        params = {'preGid': preCellGid,
        'sec': finalParam['secSynMech'],
